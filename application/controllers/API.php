    ...
    public function get_data(){
        ...
        if ($cek_auth > 1) {
            if ($cek_auth['exp_date'] > $batas && $cek_auth['status_auth'] == 1) {
                ...
                }elseif ($tipe == 'show_location') {
                    $outlet = $this->input->post('kd_outlet');
                    $data['data']  = $this->MAPI->get_data('show_location', $outlet);
                }elseif ($tipe == 'distance_location') {
        ...
        $cek_auth = $this->db->get_where("user", ["auth_key" =>$auth])->row_array();
            if ($cek_auth > 1) {
                if($tipe == 'register'){
                    ...
                    $this->MAPI->update_data('outlet', $array, 'kd_outlet', $kd_outlet);
                    $data['respon'] = [
                        'pesan' => 'Set Lokasi Berhasil',
                        'data_detail' => $array,
                        'bool' => true
                    ];
                }else{
